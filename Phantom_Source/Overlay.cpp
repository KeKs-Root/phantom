...
void CDraw::Message(char *text, float x, float y, int font, int orientation) {
	...
	switch ( orientation ) {
		case lefted:
			pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_CALCRECT | DT_LEFT, d3d9BLACK(255));

			BoxRounded(x - 5, rect.top - 5, rect.right - x + 10, rect.bottom - rect.top + 10, 5, true, d3d9DARKGRAY(150), d3d9SKYBLUE(255));

			SetRect(&rect, x, y, x, y);
			pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_LEFT | DT_NOCLIP, d3d9ORANGE(255));
			break;
		case centered:
			pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_CALCRECT | DT_CENTER, d3d9BLACK(255));

			BoxRounded(rect.left - 5, rect.top - 5, rect.right - rect.left + 10, rect.bottom - rect.top + 10, 5, true, d3d9DARKGRAY(150), d3d9SKYBLUE(255));

			SetRect(&rect, x, y, x, y);
			pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_CENTER | DT_NOCLIP, d3d9ORANGE(255));
			break;
		case righted:
			pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_CALCRECT | DT_RIGHT, d3d9BLACK(255));

			BoxRounded(rect.left - 5, rect.top - 5, rect.right - rect.left + 10, rect.bottom - rect.top + 10, 5, true, d3d9DARKGRAY(150), d3d9SKYBLUE(255));

			SetRect(&rect, x, y, x, y);
			pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_RIGHT | DT_NOCLIP, d3d9ORANGE(255));
			break;
	}
}
